...
class CustomRotatE(BaseRotatE):
    """
    A custom implementation of the RotatE model for knowledge graph embeddings,
    ...
    def __init__(self, *args, **kwargs):
        """
        Initializes the CustomRotatE model.

        Args:
            *args: Variable length argument list for base RotatE model.
    ...
    def forward(
        ...
        """
        Forward pass of the CustomRotatE model.
        ...
        """
        head_re, head_im = self.node_emb(head_index), self.node_emb_im(
            head_index
        )
        tail_re, tail_im = self.node_emb(tail_index), self.node_emb_im(
            tail_index
        )

        if (
            x is not None
            and self.feature_transform_re
            and self.feature_transform_im
        ):
    ...
    def loader(
        ...
        **kwargs,
        ...
        This method wraps the CustomKGTripletLoader to handle the specific needs of the CustomRotatE model,
