...
class PropertyConverter:
    ...
    def update_mapping(self):
        ...
        logging.info(
            "# not none mapping: %d",
            len([x for x in freebase2wikidata.values() if x is not None]),
        )
        logging.info(
            "# wikidata properties mapped: %d",
            len(set(freebase2wikidata.values())),
        )

        freebase2wikidata = {
            k.split("https://www.freebase.com")[-1]: (
                v.split("/wiki/Property:")[-1] if v is not None else v
            )
            for k, v in freebase2wikidata.items()
        }
        wikidata2freebase = {
            v: k for k, v in freebase2wikidata.items() if v is not None
        }
